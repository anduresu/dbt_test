  ...
  create  table
    "houmdw_prod".airbyte_gspread."nps_chile_sales__dbt_tmp"
    
    
      compound sortkey(_airbyte_emitted_at)
  as (
    
with __dbt__cte__nps_chile_sales_ab1 as (

-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: "houmdw_prod".airbyte_gspread._airbyte_raw_nps_chile_sales
select
...
from "houmdw_prod".airbyte_gspread._airbyte_raw_nps_chile_sales as table_alias
-- nps_chile_sales
where 1 = 1
),  __dbt__cte__nps_chile_sales_ab2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__nps_chile_sales_ab1
select
...
select
    ...
    _airbyte_emitted_at,
    getdate() as _airbyte_normalized_at,
    _airbyte_nps_chile_sales_hashid
from __dbt__cte__nps_chile_sales_ab3
-- nps_chile_sales from "houmdw_prod".airbyte_gspread._airbyte_raw_nps_chile_sales
where 1 = 1
  );