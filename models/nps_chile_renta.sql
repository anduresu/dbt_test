  ...
  create  table
    "houmdw_prod".airbyte_gspread."nps_chile_renta__dbt_tmp"
    
    
      compound sortkey(_airbyte_emitted_at)
  as (
    
with __dbt__cte__nps_chile_renta_ab1 as (

-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: "houmdw_prod".airbyte_gspread._airbyte_raw_nps_chile_renta
select
    ...
    case when json_extract_path_text(_airbyte_data, 'Octubr', true) != '' then json_extract_path_text(_airbyte_data, 'Octubr', true) end as octubr,
...
from "houmdw_prod".airbyte_gspread._airbyte_raw_nps_chile_renta as table_alias
-- nps_chile_renta
where 1 = 1
),  __dbt__cte__nps_chile_renta_ab2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__nps_chile_renta_ab1
select
    cast(date as varchar) as date,
    cast("time" as varchar) as "time",
    cast(octubr as varchar) as octubr,
    ...
    _airbyte_emitted_at,
    getdate() as _airbyte_normalized_at
from __dbt__cte__nps_chile_renta_ab1
-- nps_chile_renta
where 1 = 1
),  __dbt__cte__nps_chile_renta_ab3 as (

-- SQL model to build a hash column based on the values of this record
-- depends_on: __dbt__cte__nps_chile_renta_ab2
select
    md5(cast(coalesce(cast(date as varchar), '') || '-' || coalesce(cast("time" as varchar), '') || '-' || coalesce(cast(octubr as varchar), '') || '-' || coalesce(cast("prop id" as varchar), '') || '-' || coalesce(cast(address as varchar), '') || '-' || coalesce(cast("schedule id" as varchar), '') || '-' || coalesce(cast("¿qué tan fácil fue agendar una visita?" as varchar), '') || '-' || coalesce(cast("¿cómo evaluarías el servicio de tu houmer?" as varchar), '') || '-' || coalesce(cast("¿pudiste realizar la visita sin ningún inconveniente?" as varchar), '') || '-' || coalesce(cast("¿tienes algún comentario que nos ayude a mejorar nuestro servicio?" as varchar), '') || '-' || coalesce(cast("¿qué posibilidades hay de que recomiende nuestro servicio a un amigo o colega?" as varchar), '') || '-' || coalesce(cast("¿el houmer tenía conocimiento de la propiedad y el sector inmobiliario en general?" as varchar), '') as varchar)) as _airbyte_nps_chile_renta_hashid,
    tmp.*
from __dbt__cte__nps_chile_renta_ab2 tmp
-- nps_chile_renta
where 1 = 1
)-- Final base SQL model
-- depends_on: __dbt__cte__nps_chile_renta_ab3
select
    date,
    "time",
    octubr,
    ...
    _airbyte_emitted_at,
    getdate() as _airbyte_normalized_at,
    _airbyte_nps_chile_renta_hashid
from __dbt__cte__nps_chile_renta_ab3
-- nps_chile_renta from "houmdw_prod".airbyte_gspread._airbyte_raw_nps_chile_renta
where 1 = 1
  );