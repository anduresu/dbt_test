  ...
  create  table
    "houmdw_prod".airbyte_gspread."nps_mexico_renta__dbt_tmp"
    
    
      compound sortkey(_airbyte_emitted_at)
  as (
    
with __dbt__cte__nps_mexico_renta_ab1 as (

-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: "houmdw_prod".airbyte_gspread._airbyte_raw_nps_mexico_renta
select
...
from "houmdw_prod".airbyte_gspread._airbyte_raw_nps_mexico_renta as table_alias
-- nps_mexico_renta
where 1 = 1
),  __dbt__cte__nps_mexico_renta_ab2 as (

-- SQL model to cast each column to its adequate SQL type converted from the JSON schema type
-- depends_on: __dbt__cte__nps_mexico_renta_ab1
select
...
select
    ...
    _airbyte_emitted_at,
    getdate() as _airbyte_normalized_at,
    _airbyte_nps_mexico_renta_hashid
from __dbt__cte__nps_mexico_renta_ab3
-- nps_mexico_renta from "houmdw_prod".airbyte_gspread._airbyte_raw_nps_mexico_renta
where 1 = 1
  );